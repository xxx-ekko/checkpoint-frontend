...
const AdminPage = () => {
  ...
  // Check local storage on component mount to persist session across refreshes
  useEffect(() => {
    const authStatus = localStorage.getItem('checkpoint_admin_auth');
    if (authStatus === 'true') {
      setIsAuthenticated(true);
      fetchRecentTickets(); // Load history if already logged in
    }
  }, []);
...
const handleViewOldQr = (ticketId) => {
  ...
  // ---------------------------------------------------------------------------
  return (
    <Box sx={{ minHeight: '100vh', backgroundColor: '#131313', display: 'flex', flexDirection: 'column', alignItems: 'center', p: 4 }}>
      
      {/* Header with Logout Button */}
      <Box sx={{ width: '100%', maxWidth: 500, display: 'flex', justifyContent: 'flex-end', mb: 2 }}>
        <Button 
          onClick={handleLogout} 
          sx={{ color: '#c6c6c7', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 0, '&:hover': { color: '#9E1B1B', borderColor: '#9E1B1B' } }}
        >
          DÉCONNEXION
        </Button>
      </Box>

      <Paper sx={{ p: 4, width: '100%', maxWidth: 500, backgroundColor: '#1A1A1A', border: '1px solid #9E1B1B', borderRadius: 0 }}>
        <Typography variant="h4" sx={{ color: '#fff', mb: 1, fontFamily: '"Space Grotesk", sans-serif', textAlign: 'center' }}>
          ADMIN <span style={{ color: '#9E1B1B' }}>PORTAL</span>
        </Typography>
        <Typography variant="body2" sx={{ color: '#c6c6c7', mb: 4, textAlign: 'center' }}>
        ...
        {qrCode ? (
          ...
          <Box sx={{ textAlign: 'center', mb: 4 }}>
            <Typography variant="h6" sx={{ color: '#39FF14', mb: 2 }}>PASS PRÊT !</Typography>
            <Box sx={{ p: 2, backgroundColor: '#fff', display: 'inline-block', mb: 3 }}>
              <img src={qrCode} alt="Generated QR" style={{ width: '200px', height: '200px' }} />
            </Box>
            <Typography variant="body2" sx={{ color: '#c6c6c7', mb: 3 }}>
              Faites une capture d'écran et envoyez ce QR Code au client sur WhatsApp.
            </Typography>
            <Button 
              variant="outlined" 
              fullWidth 
              onClick={() => { setQrCode(null); setFirstName(''); setLastName(''); }}
          ...
          <form onSubmit={handleGenerateTicket}>
            <TextField
              variant="standard" fullWidth required value={firstName}
              onChange={(e) => setFirstName(e.target.value)} placeholder="Prénom (ex: Modou)"
              sx={{ mb: 3, '& .MuiInput-root': { color: '#fff', borderBottom: '2px solid rgba(255, 255, 255, 0.2)' } }}
            />
            <TextField
              variant="standard" fullWidth required value={lastName}
              onChange={(e) => setLastName(e.target.value)} placeholder="Nom (ex: Ndiaye)"
              sx={{ mb: 4, '& .MuiInput-root': { color: '#fff', borderBottom: '2px solid rgba(255, 255, 255, 0.2)' } }}
            />
            <Button type="submit" variant="contained" fullWidth disabled={loading} sx={{ backgroundColor: '#9E1B1B', height: 50, borderRadius: 0 }}>
        ...
        <Box sx={{ mt: 6 }}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
            <Typography variant="caption" sx={{ color: '#c6c6c7', letterSpacing: '0.1em' }}>
              {showTrash ? 'CORBEILLE (PASS SUPPRIMÉS)' : 'DERNIERS PASS GÉNÉRÉS'}
            </Typography>
            <Button 
              size="small" 
              onClick={() => {
          ...
          {showTrash ? (
            trashTickets.length === 0 ? (
              <Typography variant="body2" sx={{ color: '#555', fontStyle: 'italic' }}>La corbeille est vide.</Typography>
            ) : (
              trashTickets.map((ticket) => (
                <Box key={ticket.id} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: 1.5, borderBottom: '1px solid rgba(255,255,255,0.05)', backgroundColor: 'rgba(158, 27, 27, 0.05)' }}>
                  <Box>
                    <Typography variant="body1" sx={{ color: '#fff', textTransform: 'uppercase', fontSize: '0.9rem', textDecoration: 'line-through', opacity: 0.5 }}>
                      {ticket.attendeeName}
                    </Typography>
                  ...
                  <Button 
                    size="small" 
                    onClick={() => handleRestoreTicket(ticket.id, ticket.attendeeName)}
                    sx={{ color: '#39FF14', border: '1px solid rgba(57, 255, 20, 0.3)', borderRadius: 0, '&:hover': { backgroundColor: 'rgba(57, 255, 20, 0.1)' } }}
                  >
                    ♻️ RESTAURER
                  </Button>
                </Box>
            ...
            recentTickets.length === 0 ? (
              <Typography variant="body2" sx={{ color: '#555', fontStyle: 'italic' }}>Aucun pass généré pour le moment.</Typography>
            ) : (
              recentTickets.map((ticket) => (
                <Box key={ticket.id} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: 1.5, borderBottom: '1px solid rgba(255,255,255,0.05)', '&:hover': { backgroundColor: 'rgba(255,255,255,0.02)' } }}>
                  <Box>
                    <Typography variant="body1" sx={{ color: '#fff', textTransform: 'uppercase', fontSize: '0.9rem' }}>
                      {ticket.attendeeName}
                    </Typography>
                    <Typography variant="caption" sx={{ color: '#9E1B1B', fontFamily: 'monospace' }}>
                      ID: {ticket.id}
                    </Typography>
                  </Box>
                  <Box sx={{ display: 'flex', gap: 1 }}>
                    <Button 
                      ...
                      sx={{ color: '#c6c6c7', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 0, '&:hover': { backgroundColor: '#fff', color: '#000' } }}
                    >
                      VOIR QR
                    </Button>
                    <Button 
                      size="small" 
                      onClick={() => handleDeleteTicket(ticket.id, ticket.attendeeName)}
                      sx={{ color: '#ffb4ab', border: '1px solid rgba(158, 27, 27, 0.5)', borderRadius: 0, '&:hover': { backgroundColor: '#9E1B1B', color: '#fff' } }}
                    >
                      🗑️
                    </Button>
                  </Box>
